...
In doing this, you do not need to specify the input file name for **FCCDataSvc** in the configuration file. It will take automatically the ouput file **output_ecalSim_e50GeV_1events.root** of the application **simulation** as input.

Contrary to ILC applications, **setOutputFile()** method can not be use for naming the output file of the FCC application.

The name of the FCC application is specified in the configuration file (e.g. **PodioOutput().filename**).
...
(OUTPUT_FILENAME_)APPLICATIONNAME_APPLICATIONVERSION_APPLICATIONSTEP(_JobID_ID)**.**(log, root or sh)

Notice that output data are renamed automatically by appending application's name, application's version, application's step and eventually the ID of the job.
